...
const SignupComponent = () => {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8007/users/send-otp", null, {
        params: {
  ...
  const handleOtpVerification = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8007/users/validate-otp", null, {
        params: {
          email: newSignup.emailId,
          otp: enteredCode
        }
  ...
  const createUser = async () => {
    try {
      await axios.post('http://localhost:8007/users/create', newSignup);
